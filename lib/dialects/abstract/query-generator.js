...
class QueryGenerator {
  ...
  */
   ...
   * @param {string}         systemTimeQuery system time query
   *
   * @returns {string}
   */
  quoteTable(param, alias = false, systemTimeQuery = '') {
    let table = '';
  ...
  */
  ...
  /*
    ...
    Options:
      ...
      - systemTime -> The point in time to use for the system time query.
   @private
  */
  selectQuery(tableName, options, model) {
    ...
    // Get SystemTme query if supported
    if (options.systemTime) {
      mainTable.systemTime = this.generateSystemTimeQuery(options);
      if (!mainTable.systemTime) {
        throw new Error('System time is not supported by this dialect');
      }
    }

    mainTable.quotedName = !Array.isArray(mainTable.name) ? this.quoteTable(mainTable.name, false, mainTable.systemTime) : tableName.map(t => {
      return Array.isArray(t) ? this.quoteTable(t[0], t[1]) : this.quoteTable(t, true, mainTable.systemTime);
    }).join(', ');
