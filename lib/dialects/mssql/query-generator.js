...
class MSSQLQueryGenerator extends AbstractQueryGenerator {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    if (options.dialectOptions && options.dialectOptions.versioning) {
    ...
    const quotedTableHistory = this.quoteTable(`${tableName}History`);

    return Utils.joinSQLFragments([
      `IF OBJECT_ID('${quotedTableName}', 'U') IS NULL`,
      `CREATE TABLE ${quotedTableName} (${attributesClauseParts.join(', ')})`,
      options.dialectOptions && options.dialectOptions.versioning ? `WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [dbo].${quotedTableHistory}))` : '',
      ';',
      commentStr,
    ]);
  }
  ...
  dropTableQuery(tableName) {
    ...
    const quoteTblHistory = this.quoteTable(`${tableName}History`);

    return Utils.joinSQLFragments([
      `IF EXISTS (SELECT temporal_type FROM sys.tables WHERE object_id = OBJECT_ID('${quoteTbl}', 'U') AND temporal_type = 2)`,
      `ALTER TABLE ${quoteTbl} SET ( SYSTEM_VERSIONING = OFF )`,
      `IF OBJECT_ID('${quoteTblHistory}', 'U') IS NOT NULL`,
      `DROP TABLE ${quoteTblHistory};`,
  ...
  generateSystemTimeQuery(options) {
    const dbVersion = this.sequelize.options.databaseVersion;
    const versioningSupport = semver.valid(dbVersion) && semver.gte(dbVersion, '13.0.0');

    if (versioningSupport && this.options.dialectOptions && this.options.dialectOptions.versioning && options.systemTime) {
      const { type, startDate, endDate } = options.systemTime;
      let systemTimeQuery = 'FOR SYSTEM_TIME';
      if (type === 'ASOF' && startDate && startDate instanceof Date) {
        const date = new Date(startDate);
        date.setUTCMilliseconds(date.getUTCMilliseconds() + 1);
        systemTimeQuery += ` AS OF ${this.escape(date.toISOString())}`;
      } else if (type === 'FROM' && startDate && endDate && startDate instanceof Date && endDate instanceof Date) {
        systemTimeQuery += ` FROM ${this.escape(options.systemTime.startDate.toISOString())} TO ${this.escape(options.systemTime.endDate.toISOString())}`;
      } else if (type === 'BETWEEN' && startDate && endDate && startDate instanceof Date && endDate instanceof Date) {
        systemTimeQuery += ` BETWEEN ${this.escape(options.systemTime.startDate.toISOString())} AND ${this.escape(options.systemTime.endDate.toISOString())}`;
      } else if (type === 'CONTAINED' && startDate && endDate && startDate instanceof Date && endDate instanceof Date) {
        systemTimeQuery += ` CONTAINED IN (${this.escape(options.systemTime.startDate.toISOString())}, ${this.escape(options.systemTime.endDate.toISOString())})`;
      } else if (type === 'ALL') {
        systemTimeQuery += ' ALL';
      }

      return systemTimeQuery;
    }
